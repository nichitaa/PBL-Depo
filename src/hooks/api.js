...
export async function newRequestsListeners(currentUserUID = null) {
	const unsubscribe = await db
		.collection(COLLECTIONS.PROJECTS_REQUESTS)
...
export async function deleteProject(Id) {
	...
	const moveToDeletedCollection = async () => {
		// get project data
		await getProjectById(Id).then(async ({ project, feedback }) => {
			// update DeletedProjects collection
			return await db
				.collection(COLLECTIONS.DELETED_PROJECTS)
	...
	moveToDeletedCollection().then(async () => {
		...
		return await db
			...
			.then(async () => {
				await db
					.collection(COLLECTIONS.PROJECTS) // second -> delete the project data
					.doc(Id)
					.delete()
					.then(async () => {
						await db
							.collection(COLLECTIONS.USERS_PROJECTS) // third -> delete form users projects
							.doc(userId)
...
export async function sendFeedback(project, feedback, rating, user) {
	...
	return await docRef
		.set({
			// set, because users one user can not have multiple feedbacks on the same project
			...
			[FIELDS.FEEDBACK_CREATEDAT]: new Date(),
		})
		.then(async () => {
			// update rating value
			updateRating().then();
...
export function getStats(setStats) {
	db.collection(COLLECTIONS.PROJECTS).onSnapshot((snapshot) => {
